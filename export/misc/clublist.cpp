...
void ClubList::printContent() {
    ...
    auto eventId = m_event->mainEvent()->id();
    ...
    query2.bindValue( 1, eventId );
    query2.bindValue( 2, eventId );
    query2.bindValue( 3, eventId );
    query2.exec();
    while (query2.next()) {
        ...
        setPrinterFont( 10 );
        if( query2.at() % 2 != 0 ){
            drawHighlightRect( m_yco, mmToPixel(5.0) );
        }
        painter.drawText(QRectF(pr.x(), m_yco, mmToPixel(79.4), QFontMetricsF(painter.font()).height()),QString(query2.value(0).toString() + "  "),QTextOption(Qt::AlignVCenter));
        painter.drawText(QRectF(pr.x()+mmToPixel(79.4), m_yco,mmToPixel(26.5), QFontMetricsF(painter.font()).height()),QString(query2.value(1).toString()),QTextOption(Qt::AlignVCenter));
        if (query2.value(2).toInt() > 0) painter.drawText(QRectF(pr.x()+mmToPixel(105.9), m_yco, mmToPixel(26.5), QFontMetricsF(painter.font()).height()),QString(query2.value(2).toString()),QTextOption(Qt::AlignVCenter));
        if (query2.value(3).toInt() > 0) painter.drawText(QRectF(pr.x()+mmToPixel(132.4), m_yco, mmToPixel(26.5), QFontMetricsF(painter.font()).height()),QString(query2.value(3).toString()),QTextOption(Qt::AlignVCenter));
        m_yco += mmToPixel(5.3);
    }
    finishPrint();
}
...
void ClubList::printSubHeader() {
    ...
    painter.drawText(QRectF(pr.x()+mmToPixel(79.4), m_yco, mmToPixel(26.5), QFontMetricsF(painter.font()).height()),"Teilnehmer",QTextOption(Qt::AlignVCenter));
    QSqlQuery query;
    ...
    query.exec();
    query.next();
    if (query.value(0).toInt() > 0) painter.drawText(QRectF(pr.x()+mmToPixel(105.9), m_yco, mmToPixel(26.5), QFontMetricsF(painter.font()).height()),"Mannschaften",QTextOption(Qt::AlignVCenter));
    if (query.value(1).toInt() > 0) painter.drawText(QRectF(pr.x()+mmToPixel(132.4), m_yco, mmToPixel(26.5), QFontMetricsF(painter.font()).height()),"Gruppen",QTextOption(Qt::AlignVCenter));
    m_yco += mmToPixel(5.0);
