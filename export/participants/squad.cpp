...
void Squad::printContent() {
    ...
    for( auto& currRiege : riegenNumbers ) {
        ...
        if( ( max_yco - m_yco - mmToPixel( 55.0 ) < 0 ) || ( newPageEach && !bFirstItem ) ){
          newPage();
        }

        setPrinterFont( 18, true );
        painter.drawText( QRectF(pr.x(), m_yco, pr.width()-pr.x()-pr.x(), (QFontMetricsF(painter.font()).height())),"Riege "+currRiege,QTextOption(Qt::AlignVCenter | Qt::AlignHCenter));
        m_yco += QFontMetricsF(painter.font()).height();
        ...
        if (_global::querySize(query3) > 0) {
            ...
            painter.drawText(QRectF(pr.x(), m_yco, pr.width()-pr.x()-pr.x(), (QFontMetricsF(painter.font()).height())),"1. Gerät: " + query3.value(0).toString(),QTextOption(Qt::AlignHCenter | Qt::AlignVCenter));
            m_yco += QFontMetricsF(painter.font()).height();
        }
        m_yco += mmToPixel(5.0);
