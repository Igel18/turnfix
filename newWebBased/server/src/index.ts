...
const envFilePath = isAbsolute(requestedEnvFile)
  ? requestedEnvFile
  : resolve(__dirname, '..', requestedEnvFile);
...
if (process.env.NODE_ENV === 'production') {
  ...
  // NOTE: Jury Portal is NOT served on port 3001.
  // It runs exclusively on port 3002 via the standalone jury-server process.
  // This ensures that only port 3002 needs to be opened in the firewall for judges,
  // keeping the management UI on port 3001 protected.
  ...
  app.get('*', (req, res, next) => {
    ...
    // Skip API routes, uploads, and public files
