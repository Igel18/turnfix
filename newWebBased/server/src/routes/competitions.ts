...
const createCompetitionSchema = z.object({
  ...
  areaId: z.number().nullable().optional(),          // int_bereicheid - direct Bereich selection
  ...
  try {
    ...
    const transformedCompetitions = competitions.map((comp: any) => {
      ...
      return {
        ...
        disciplines: comp.tfx_wettkaempfe_x_disziplinen.map((wd: any) => ({
    ...
    transformedCompetitions.slice(0, 5).forEach((comp: any) => {
      const originalComp = competitions.find((c: any) => c.int_wettkaempfeid === comp.id);
