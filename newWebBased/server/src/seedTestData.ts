...
async function seedTestData() {
  try {
    ...
    const clubCount = await prisma.$queryRaw`SELECT COUNT(*) as count FROM tfx_vereine` as Array<{count: bigint}>;
    const participantCount = await prisma.$queryRaw`SELECT COUNT(*) as count FROM tfx_teilnehmer` as Array<{count: bigint}>;
