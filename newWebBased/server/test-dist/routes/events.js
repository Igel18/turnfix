...
router.get('/:id/statistics', authBypass_1.authenticateToken, async (req, res) => {
    ...
    `;
      ...
      SELECT 
        g.var_name as discipline_name,
        COUNT(DISTINCT w.int_teilnehmerid) as participant_count
      FROM tfx_wertungen w
      INNER JOIN tfx_wettkaempfe wk ON w.int_wettkaempfeid = wk.int_wettkaempfeid
      INNER JOIN tfx_geraete g ON wk.int_geraeteid = g.int_geraeteid
      WHERE wk.int_veranstaltungenid = $1
      GROUP BY g.int_geraeteid, g.var_name
    ...
    `;
      ...
      SELECT 
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 1 THEN w.int_teilnehmerid END) as "1_6",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 2 THEN w.int_teilnehmerid END) as "7_8",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 3 THEN w.int_teilnehmerid END) as "9_10",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 4 THEN w.int_teilnehmerid END) as "11_12",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 5 THEN w.int_teilnehmerid END) as "13_14",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 6 THEN w.int_teilnehmerid END) as "15_16",
        COUNT(DISTINCT CASE WHEN t.int_altersklasseid = 7 THEN w.int_teilnehmerid END) as "17_18"
      FROM tfx_wertungen w
      INNER JOIN tfx_wettkaempfe wk ON w.int_wettkaempfeid = wk.int_wettkaempfeid
      INNER JOIN tfx_teilnehmer t ON w.int_teilnehmerid = t.int_teilnehmerid
      WHERE wk.int_veranstaltungenid = $1
    `;
        ...
        const result = {
            total_participants: Number(stats.total_participants),
            male_participants: Number(stats.male_participants),
            female_participants: Number(stats.female_participants),
            total_clubs: Number(stats.total_clubs),
            total_competitions: Number(competitions.total_competitions),
