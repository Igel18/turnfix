// Check Event 39 - Schüler- und Jugendbestenkämpfe (Fixed column names)
...
async function checkEvent39() {
  try {
    ...
    // Find participants in Event 39 - fix column names
    ...
    for (const p of participants) {
      console.log(`\n👤 ${p.var_vorname} ${p.var_nachname} (ID: ${p.int_teilnehmerid})`);
    ...
    if (mirja.length > 0) {
      ...
      const mirjaExists = await prisma.$queryRawUnsafe(`
        ...
        FROM tfx_teilnehmer 
        WHERE var_nachname LIKE '%Baretti%'
      `);
      
      if (mirjaExists.length > 0) {
        console.log('🔍 But Mirja Baretti exists in the database:');
        mirjaExists.forEach(p => {
          console.log(`   ${p.var_vorname} ${p.var_nachname} (ID: ${p.int_teilnehmerid})`);
        });
        
        // Check in which events Mirja participates
        const mirjaEvents = await prisma.$queryRawUnsafe(`
          SELECT DISTINCT
            wk.int_veranstaltungenid,
            v.str_titel as event_title,
            w.int_wertungenid
          FROM tfx_teilnehmer t
          INNER JOIN tfx_wertungen w ON t.int_teilnehmerid = w.int_teilnehmerid
          INNER JOIN tfx_wettkaempfe wk ON w.int_wettkaempfeid = wk.int_wettkaempfeid
          INNER JOIN tfx_veranstaltungen v ON wk.int_veranstaltungenid = v.int_veranstaltungenid
          WHERE t.var_nachname LIKE '%Baretti%'
          ORDER BY wk.int_veranstaltungenid
        `);
        
        console.log(`\n📋 Mirja Baretti participates in ${mirjaEvents.length} events:`);
        mirjaEvents.forEach(event => {
          console.log(`   Event ${event.int_veranstaltungenid}: ${event.event_title} (WertungenId: ${event.int_wertungenid})`);
        });
      }
    }
