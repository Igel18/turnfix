...
const DisciplinesPage: React.FC = () => {
  ...
  const getFilterOptions = () => [
    {
      ...
      options: [
        ...
        ...sports.map(sport => ({ value: sport.int_sportid.toString(), label: sport.var_name }))
      ]
    },
    {
      value: '',
  ...
  // Render table row
  const renderTableRow = (discipline: Discipline, index: number) => {
    const sport = sports.find(s => s.int_sportid === discipline.sport_id);
    const formula = formulas.find(f => f.int_formelid === discipline.formula_id);
