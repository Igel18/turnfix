 ...
 * 
 * BASED ON: ScoreCapture/hooks/useFormulaCalculation.ts (proven implementation)
 * UPDATED: Uses expr-eval for CSP-safe formula evaluation
 * 
 * Used by:
 * - Disciplines page (formula testing)
 * - Score Capture (formula-based input)
 * - Results view (formula display)
 * - PDF Export (formula in exports)
 ...
 * 
 * BASED ON: parseFormulaDisplay from useFormulaCalculation
 */
 ...
 * 
 * UPDATED: Uses expr-eval for CSP-safe evaluation (no Function() constructor)
 * BASED ON: evaluateFormula from useFormulaCalculation (proven implementation)
 */
  ...
  try {
    ...
    FORMULA_VARIABLES.forEach(variable => {
      if (values[variable] !== undefined) {
        ...
        // Replace undefined variables with 0 (same as ScoreCapture)
        const regex = new RegExp(`\\b${variable}\\b`, 'g');
    ...
    // Use CSP-safe expr-eval parser instead of Function() constructor
