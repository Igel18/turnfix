/**
 * Master Data Tests — Formulas (Formeln)
 *
...
const TS = Date.now();
const TEST_NAME = `E2E_Formel_${TS}`;
const EDITED_NAME = `${TEST_NAME}_ed`;

test.describe.serial('Master Data: Formulas', () => {

  test('page loads with table or empty state', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
    const hasTable = (await page.locator('table').count()) > 0;
    const hasEmpty = (await page.getByText(/keine|leer|empty|no data|no formula/i).count()) > 0;
  ...
  test('has correct page title', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
    await expect(page.locator('h1, h2').first()).toContainText(/Formel|Formula/i);
  });
  ...
  test('required fields are marked in form', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
  ...
  test('can create formula', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
    ...
    // Fill formula name (first text input)
    const nameInput = modal.locator('input[type="text"]').first();
    await nameInput.fill(TEST_NAME);

    // Fill formula code if present (textarea or second input)
    const codeInput = modal.locator('textarea, input[type="text"]').nth(1);
    if (await codeInput.isVisible({ timeout: 500 }).catch(() => false)) {
      await codeInput.fill('A + B');
    }

    const submitButton = modal.getByRole('button', { name: /erstellen|create|speichern|save/i });
    const [response] = await Promise.all([
      page.waitForResponse(r => r.url().includes('/api/formulas') && r.request().method() === 'POST'),
      submitButton.click()
    ]);
  ...
  test('can edit formula', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
    await openFilterAndSearch(page, TEST_NAME);
    ...
    const [response] = await Promise.all([
      page.waitForResponse(r => r.url().includes('/api/formulas') && r.request().method() === 'PUT'),
      saveButton.click()
    ]);
  ...
  test('can delete formula and confirm via modal', async ({ page }) => {
    await navigateTo(page, '/formulas');
    await waitForLoadingToFinish(page);
    await openFilterAndSearch(page, EDITED_NAME);
